namespace OmieSharp.IntegrationTests;

public class Constants
{
    public const string CLIENTE_CODIGO_INTEGRACAO = "20221202_230906_358";
    public const string CLIENTE_EMAIL = "teste@teste.com.br";
    public const string CLIENTE_RAZAO_SOCIAL = "Teste OmieSharp LTDA";
    public const string CLIENTE_NOME_FANTASIA = "Teste OmieSharp";
    public const string CLIENTE_CNPJ = "12121212000106";
    
    public const string FORNECEDOR_CODIGO_INTEGRACAO = "20230415_155400";
    public const string FORNECEDOR_EMAIL = "fornecedor@teste.com.br";
    public const string FORNECEDOR_RAZAO_SOCIAL = "Fornecedor Teste OmieSharp LTDA";
    public const string FORNECEDOR_NOME_FANTASIA = "Fornecedor Teste OmieSharp";
    public const string FORNECEDOR_CNPJ = "06142444000148";

    public const string SERVICO_CODIGO_INTEGRACAO = "TesteOmieSharp_001";

    public const string CONTA_CORRENTE_CODIGO_INTEGRACAO = "TesteOmieSharp_001";
}
